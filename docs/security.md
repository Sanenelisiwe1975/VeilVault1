...
- `add_signer` is exposed as "add backup passkey" via `/api/passkey/signers/*` (`backend/src/services/passkey.service.ts`, `backend/src/api/routes/passkey-signer.routes.ts`), and **any** contract method requiring this wallet's auth — not just `add_signer` — is exposed generically via `/api/passkey/tx/prepare` + `/api/passkey/tx/submit` (`backend/src/api/routes/passkey-tx.routes.ts`). Both sets of routes require an authenticated session and read the wallet address from `req.walletAddress` (set by `apiKeyAuth` from the validated session token) rather than trusting a client-supplied address — a caller can only ever authorize a transaction as their own wallet. The generic routes accept the contract method's args as a small typed union (`u32`/`u64`/`i128`/`bytes`/`address`/`string`/`symbol`/`bool` — `backend/src/utils/scval-args.ts`), built into ScVals server-side; they do **not** accept raw client-supplied XDR for args, to keep the encoding auditable in one place. The backend builds the `SorobanAuthorizationEntry` manually (`StellarClient.prepareAuthorizedInvocation` / `submitAuthorizedInvocation`), since the SDK's own `authorizeEntry` helpers hard-code Ed25519 verification and can't produce the contract's custom `WebAuthnSignature` credential shape. The add-backup-passkey routes are themselves thin wrappers around the same `startPasskeyTransaction` / `finishPasskeyTransaction` core the generic routes use — not a parallel implementation. Validated end to end against live testnet (`backend/scripts/validate-passkey-tx-auth.ts`), both for `add_signer` (via the lower-level `StellarClient` methods directly) and `remove_signer` (via the generic service-layer functions, including `@simplewebauthn/server`'s own off-chain assertion verification — not just the on-chain check).
...
- A second gotcha from that same validation: `nativeToScVal`'s symbol-keyed map encoding for `WebAuthnSignature` left `signer_index`'s value type unspecified, so it was auto-inferred as `u64` instead of the contract's `u32` field — the contract's struct deserializer trapped (`VM call trapped: UnreachableCodeReached`) on the mismatch with no clean error surfaced. Any struct-typed ScVal built this way needs every scalar field's type explicit, not just the map keys' `'symbol'` tag.
- A third gotcha, found validating the *generic* path specifically: the fee-payer's account sequence number baked into the prepared transaction at prepare time can go stale by submit time — the WebAuthn ceremony in between is user-paced, and a shared fee-payer (e.g. the backend's admin key, used across many users' transactions) may advance its sequence via unrelated transactions in the meantime. `submitAuthorizedInvocation` now re-loads the fee payer's account fresh at submit time rather than reusing the sequence number from the original prepared envelope.
...
- [ ] Independent review of `StellarClient.prepareAuthorizedInvocation` / `submitAuthorizedInvocation`, the DER-decode + low-S normalization in `backend/src/utils/ecdsa.ts`, and the generic `/api/passkey/tx/*` routes' arg-type encoding (`backend/src/utils/scval-args.ts`) — these now authorize arbitrary real transactions for any passkey wallet, validated against testnet (`add_signer` and `remove_signer`) but not yet externally reviewed
- [ ] Wire passkey-wallet transaction authorization into the app's other transaction-building flows (privacy-pool deposits, stokvel votes, etc.) — the generic mechanism exists end to end (`WalletSession.invokeAsPasskeyWallet`), but those flows still only call `signTransaction`, which throws for passkey wallets
